...
<template>
  <BaseModal :isOpen="true" @close="emit('close')" :modalAriaLabel="'Deletar Produto'" :modalAriaLabelledby="'Deletar Produto'" :modalAriaDescribedby="'Deletar Produto'" :closeButtonLabel="'Deletar Produto'">
    ...
    <template #footer data-slot="modal-footer">
      <BaseButton variant="default" @click="emit('close')" class="w-full m-auto md:w-1/2 h-12 flex justify-center items-center gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
        </svg>
        Cancelar
      </BaseButton>
      <BaseButton variant="danger" @click="emit('delete')" class="w-full m-auto md:w-1/2 h-12 flex justify-center items-center gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
